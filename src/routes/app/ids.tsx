...
function Ids() {
  ...
  return (
    <div>
      <PageHeader title="My IDs" hint="Switching the active ID updates dashboard, team, levels, and qualification for that ID only." />
      <div className="grid gap-3 lg:grid-cols-2">
        {ids.data.map((row) => {
          const land = evaluateLandQualification({
            hasMembership: row.placement_status !== "pending_config",
            directSponsors: row.directSponsors,
          ...
          return (
            <Card key={row.id} className={row.id === active ? "shadow-[0_0_0_2px_var(--color-accent)]" : undefined}>
              ...
              <dl className="mt-4 grid grid-cols-2 gap-x-3 gap-y-2 text-sm">
                ...
                <div>
                  <dt className="text-xs text-muted">Placement parent</dt>
                  <dd className="font-mono text-xs">{row.parent_id ?? "—"}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted">Current level</dt>
                  <dd>Level {row.currentLevel}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted">Land qualification</dt>
                  <dd>{land.status}</dd>
                </div>
                <div>
                  <dt className="text-xs text-muted">Held commission</dt>
                  <dd>
                    <Money amount={row.held} size="sm" />
                  </dd>
                </div>
                <div>
                  <dt className="text-xs text-muted">Available commission</dt>
                  <dd>
                ...
                <div>
                  <dt className="text-xs text-muted">Direct sponsors</dt>
                  <dd className="tabular">
                    {row.directSponsors} / 3
                  </dd>
                </div>
              </dl>
