...
# Node server artifact (Nitro node-server) — not Vercel/.vercel/output.
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=40s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:8080/api/health').then(r=>process.exit(r.ok||r.status===503?0:1)).catch(()=>process.exit(1))"
